/*
 * $Id: TransactionWorker.java,v 1.5 2011/05/09 07:50:37 zhourumin Exp $
 *
...
public class TransactionWorker extends Thread implements EventListener{
	...
	/** The logger */
	private static final Logger log = Logger.getLogger(TransactionWorker.class);
	...
	private synchronized DiameterTransaction addTransaction(DiameterMessage req,TransactionListener tl,boolean blocking)
	{
		...
		dt.expires = System.currentTimeMillis() + this.timeout;
		transactions.add(dt);
		return dt;
	}
	...
	private synchronized void timeoutExpired(long now)
	{
		...
		for(i=0;i<transactions.size();i++){
			...
			long deltaTime = now - dt.expires;
			//log.error("the delta time:" + deltaTime);
...
//	    		}
			    ...
			    log.error("the request time out:\n" + dt.request);
	...
	public boolean sendRequestTransactional(String peerFQDN,DiameterMessage req,TransactionListener tl)
	{
		...
		DiameterTransaction dt;
		if (!req.flagRequest) {
			log.error("DiameterTransaction:sendMessageTransactional() is only for Requests!");
			return false;
		}
	...
	public DiameterMessage sendRequestBlocking(String peerFQDN,DiameterMessage req)
	{
		...
		DiameterTransaction dt;
		if (!req.flagRequest) {
			log.error("DiameterTransaction:sendMessageBlocking() is only for Requests!");
			return null;
		}
		...
		if (!sent){
			log.error("send the request error:/n" + req);
			takeTransaction(req);
			return null;
		}
		...
		try {
			...
			dt.wait( this.timeout);
			long afterTime =  System.currentTimeMillis();
			waitTime = afterTime - beforeTime;			
		}
		...
		if(dt.answer == null){
		    ...
		    sb.append("the request timeout time:(").append(waitTime).append(")\n");
		    sb.append("the request msg: \n" + req);
	...
	public DiameterMessage sendRequestBlocking(DiameterMessage req)
	{
		...
		dt = addTransaction(req,null,true);
		sent = dp.sendMessage(req);
		log.debug("send request result:" + sent);
		
		if (!sent){
		     log.error("Cann't send the request:/n" + req);	
			takeTransaction(req);
			return null;
		}
		dt.expires = req.networkTime + this.timeout;
...
//			}
	...
	public void recvMessage(String FQDN, DiameterMessage msg) 
	{
	      ...
	      StringBuilder sb = new StringBuilder();
	       sb.append("match diameter msg:hopByHopID=").append(msg.hopByHopID).append(":endToEndID=").append(msg.endToEndID);
	     //  log.error(sb.toString()); 
		...
		if (dt == null){
		    ...
		    info.append("the response's request has been time out:\n").append(msg);
