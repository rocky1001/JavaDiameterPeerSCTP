/*
 * $Id: StateMachine.java,v 1.3 2011/06/01 17:51:08 qiwenyuan Exp $
 * 
...
public class StateMachine {
     ...
     * 
     * @param p
     *            The peer for which the event happened.
     * @param event
     *            The event happened.
     ...
     * 
     * @param p
     *            The peer for which the event happend.
     * @param event
     *            The event happened.
     * @param msg
     *            Received message.
     ...
     *            Received message.
     * @param comm
     *            Communicator used to send a DiameterMessage
     * @return 1 one success, 0 one error. Also the peer states are upated.
     */
    public static int process(Peer p, int event, DiameterMessage msg, Communicator comm) {
        ...
        // log.debug("Peer Old State=" + p.state + " FQDN=" + p.FQDN);
        // log.debug("Now event=" + event + " FQDN=" + p.FQDN);
        synchronized (p) {
            switch (p.state) {
                case Closed:
                    switch (event) {
                        ...
                        case Stop:
                            ...
                            break;
                        default:
                            log.error("StateMachine: Invalid event " + event + " for state " + p.state);
                            return 0;
                    }
                    break;
                case Wait_Conn_Ack:
                ...
                case Closing:
                    ...
                    break;
                default:
                    log.error("StateMachine: Invalid state " + p.state);
                    return 0;
    ...
    private static int I_Snd_Conn_Req(Peer p) {
        SctpChannel sc = null;
        String bindLocalIp = p.localIp;
        int bindLocalPort = p.localPort;
        try {
            InetAddress bindLocalAdd = InetAddress.getByName(bindLocalIp);
            InetSocketAddress bindSocketAdd = new InetSocketAddress(bindLocalAdd, bindLocalPort);
            sc = SctpChannel.open();
            sc.bind(bindSocketAdd);
        } catch (IOException e) {
            log.error("DiameterPeer: Can not resolve " + bindLocalIp);
            log.error(e, e);
        }
        ...
        if (log.isInfoEnabled()) {
            ...
            sb.append("Start connect the peer:").append(p.FQDN).append(":").append(p.port);
            log.info(sb.toString());
        }

        InetSocketAddress serverAddr = new InetSocketAddress(p.FQDN, p.port);
        try {
            if (sc.connect(serverAddr, 0, 0)) {
                Communicator r = new Communicator(sc, p, Communicator.Initiator);
                p.I_comm = r;
                log.info("Remote peer <" + p.FQDN + ":" + p.port + "> connectted !");
                return StateMachine.I_Rcv_Conn_Ack;
            } else {
                log.error("Connect return false:Remote peer <" + p.FQDN + ":" + p.port + "> not responding to hss connection attempt !");
                return StateMachine.I_Rcv_Conn_NAck;
            }
        } catch (IOException e1) {
            log.error("IOException:Remote peer <" + p.FQDN + ":" + p.port + "> not responding to hss connection attempt !");
            return StateMachine.I_Rcv_Conn_NAck;
        }
    }
    ...
    private static void I_Snd_CER(Peer p) {
        ...
        Snd_CE_add_hostIpAddress(cer, p);
    ...
    private static int Process_CER(Peer p, DiameterMessage cer) {
        log.debug("Now processing CER msg for peer:" + p);
        int common_app = 0;
        ...
        while (i.hasNext() && common_app == 0) {
            ...
            switch (avp.code) {
                case AVP.Auth_Application_Id:
                    ...
                    while (i2.hasNext()) {
                        app = i2.next();
                        if (avp.int_data == Application.Relay || (app.id == avp.int_data && app.vendor == 0)) {
                            common_app++;
                            break;
                        }
                    }
                ...
                case AVP.Vendor_Specific_Application_Id:
                    ...
                    if (avp2 != null) {
                        ...
                        while (i2.hasNext()) {
                            app = i2.next();
                            if (avp2.int_data == Application.Relay || (app.id == avp2.int_data && app.vendor == avp_vendor.int_data)) {
                                common_app++;
                                break;
                            }
                        }
        ...
        if (common_app != 0) {
            log.debug("Finished processing CER msg for peer:" + p + " result is:" + DiameterMessage.DIAMETER_SUCCESS);
            return DiameterMessage.DIAMETER_SUCCESS;
        } else {
            log.debug("Finished processing CER msg for peer:" + p + " result is:" + DiameterMessage.DIAMETER_NO_COMMON_APPLICATION);
            return DiameterMessage.DIAMETER_NO_COMMON_APPLICATION;
        }
    }
    ...
    private static int Process_CEA(Peer p, DiameterMessage cea) {
        ...
        if (avp == null) {
            return DiameterMessage.DIAMETER_UNABLE_TO_COMPLY;
        } else {
            return avp.int_data;
        }
    }
    ...
    private static void Snd_CEA(Peer p, DiameterMessage cer, int result_code, Communicator comm) {
        ...
        byte addr[] = null;
        byte laddr[] = convertIpAddToByteArray(p.diameterPeer.bindIp);
        switch (laddr.length) {
            case 16:
                addr = new byte[18];
                addr[0] = (byte) 0;
    ...
    private static void R_Disc(Peer p) {
        log.debug("Now doing disconnect the peer:" + p);
        if (p.R_comm != null) {
            p.R_comm.peer = null;
            p.R_comm.shutdown();
        }
        p.R_comm = null;
        log.debug("Finished disconnect the peer:" + p);
    }
    ...
    private static void Snd_CE_add_hostIpAddress(DiameterMessage msg, Peer p) {
        Iterator<String> i = p.diameterPeer.hostIpAdds.iterator();
        String hostIpAddress = null;
        while (i.hasNext()) {
            hostIpAddress = i.next();
            byte addr[] = null;
            byte laddr[] = convertIpAddToByteArray(hostIpAddress);
            if (null != laddr) {
                switch (laddr.length) {
                    case 16:
                        addr = new byte[18];
                ...
                AVP avp = new AVP(AVP.Host_IP_Address, true, 0);
                avp.setData(addr);
                msg.addAVP(avp);
            }
        }
     ...
     * 
     * @param p
     *            Peer sending the Diameter message.
     * @param msg
     *            Diameter message being sent.
     */
